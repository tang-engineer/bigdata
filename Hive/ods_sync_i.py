...
# 配置日志记录器
log_filename = os.path.join('/home/big-data-sync/ods_sync/log', datetime.now().strftime('ods_sync_i_%Y%m%d_%H%M%S.log'))
logging.basicConfig(
...
def migrate_data(src_db, dest_db, table_name, incremental_date=None):
    ...
    # 判断表师傅是分区表
    is_parttion_table = execute_beeline_command("""SHOW PARTITIONS {}.{};""".format(src_db,table_name))
    # 当返回行数是2时，表示库中已经存在此表
    if result > 1:
        if '_i' in table_name and is_parttion_table is not None:
            logging.info("目标表 {}.{} 已存在且是分区表，先删除分区数据，再使用 INSERT INTO 插入数据。".format(dest_db, table_name))
            migration_sql = """ALTER TABLE {}.{} DROP IF EXISTS PARTITION (etl_date = \'{}\');
                                INSERT INTO  TABLE {}.{}  SELECT * FROM {}.{} ;
                            """.format(dest_db, table_name, incremental_date,dest_db, table_name, src_db, table_name)

            if where_clause and '_i' in table_name:
                migration_sql = migration_sql.replace(" ;", " {}".format(where_clause) + ";")
                logging.info(migration_sql)
        ...
        migration_sql = """
            ...
            SELECT * FROM {}.{};
        """.format(dest_db, table_name, src_db, table_name)
        result4 = execute_beeline_command_application_id(migration_sql)
        return result4
